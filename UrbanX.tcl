...
exec /software/cmc/eer_SPI-7.2.3/tclsh "$0" "$@"
load /software/cmc/eer_SPI-7.2.3/Shared/$env(ARCH)/libTkViewport.so TclData
#exec /software/cmc/eer_SPI-7.2.1/tclsh "$0" "$@"
#load /software/cmc/eer_SPI-7.2.1/Shared/$env(ARCH)/libTkViewport.so TclData
#*** if 7.2.1 THEN CHANGE Data(Mode) to INCLUDED
...
# Dorval, Quebec
#
# Projet     : CRTI - Canyon Urbain
# Fichier    : autoteb_v[X].tcl
...
#              données BNDT 1:50000, SRTM-DEM, DNEC1 et StatCan (population),
...
namespace eval AutoTEB { } {
   variable Data
   set Data(AutoTEB_version)  v1.41666  ;# date = 2006-09-29
   ...
   set Data(Res) 5                  ;# Conserver à 5 m sauf pour tests
   set Data(Mode) FAST              ;# INCLUDED or FAST: ALWAYS USE FAST FROM NOW ON   
...
proc AutoTEB::AreaParameters { } {
   ...
   if { $Data(Zone_name) == "van" } {
      ...
      set Data(Hauteur_file) /home/afsr/alx/eveil/canyon-urbain/global_data/srtm-dnec/srtm-dnec_van_latlong
      set Data(Hauteur_file_wmask) /home/afsr/alx/eveil/canyon-urbain/global_data/srtm-dnec/srtm-dnec_van_latlong_wmask
   } elseif { $Data(Zone_name) == "mtl" } {
      set Data(maxlon)   -73.35
      set Data(maxlat)    45.70
      set Data(minlon)   -73.98
      set Data(minlat)    45.30
      set Data(Hauteur_file) /home/afsr/alx/eveil/canyon-urbain/global_data/srtm-dnec/mtl_dnec_-_srtm_utm5m_cropped
      set Data(Hauteur_file_wmask) /home/afsr/alx/eveil/canyon-urbain/global_data/srtm-dnec/mtl_dnec_-_srtm_utm5m_cropped_wmask   
      ...
      set Data(Hauteur_file) /home/afsr/alx/eveil/canyon-urbain/global_data/srtm-dnec/srtm-dnec_van_latlong
      set Data(Hauteur_file_wmask) /home/afsr/alx/eveil/canyon-urbain/global_data/srtm-dnec/srtm-dnec_van_latlong
   } elseif { $Data(Zone_name) == "tor" } {
      set Data(maxlon)   -79.12
      set Data(maxlat)    43.92
      set Data(minlon)   -79.85
      set Data(minlat)    43.49
      set Data(Hauteur_file) /home/afsr/alx/eveil/canyon-urbain/global_data/srtm-dnec/srtm-dnec_van_latlong ;# TO UPDATE ****
      set Data(Hauteur_file_wmask) /home/afsr/alx/eveil/canyon-urbain/global_data/srtm-dnec/srtm-dnec_van_latlong
      ...
      set Data(minlat)    45.30
      set Data(Hauteur_file) /home/afsr/alx/eveil/canyon-urbain/global_data/cities/ottawa/ott_shp-height.tif
      set Data(Hauteur_file_wmask) /home/afsr/alx/eveil/canyon-urbain/global_data/cities/ottawa/ott_shp-height.tif
...
proc AutoTEB::FindZoneUTM { } {
   variable Data
   puts "AutoTEB Version $Data(AutoTEB_version)"
...
proc AutoTEB::FindNTSSheets { } {
   variable Data
   set Data(NTS_index) /data/eer1/DBRas/Index/50kindex.shp
...
proc AutoTEB::FindPaths { } {
   variable Data
   set j 0
   set i 0
   foreach file $Data(Sheets) {
      set file [string tolower $file]
      set Data(Path$i)    /data/eveil/afsralx/canyon-urbain/global_data/bndt-geonet/$file/
...
#               puts "*** WARNING *** FILE NAMED '$Data(Sheet_names$i)_$file.shp' HAS NO PRIORITY VALUE AND WON'T BE PROCESSED ****\n"
   ...
   puts "Writing results to $Data(Zone_name)_sandwich_$Data(Res)m_$Data(AutoTEB_version).tif"
   file delete -force $Data(Zone_name)_sandwich_$Data(Res)m_$Data(AutoTEB_version).tif
   gdalfile open FILEOUT write $Data(Zone_name)_sandwich_$Data(Res)m_$Data(AutoTEB_version).tif GeoTiff
   gdalband write RSANDWICH FILEOUT { COMPRESS NONE PROFILE GeoTIFF }
   gdalfile close FILEOUT
   gdalband free RSANDWICH
}

proc AutoTEB::ScaleBuffers { } {
   variable Data
   set Data(Buffer_layers)   { bridge_l buildin_p road_l }
   puts "\nBuffers for scale representation at resolution <= 5m ($Data(Buffer_layers))"
   gdalband read RSANDWICH [gdalfile open FSANDWICH read $Data(Zone_name)_sandwich_$Data(Res)m_$Data(AutoTEB_version).tif]
      ...
      foreach layer $Data(Buffer_layers) {
         ...
         if { [file exists $path] } {
            ...
            if  { $layer=="buildin_p" }  {
               ...
               #ogrlayer sqlselect LAYER$j SHAPE " SELECT * FROM $Data(Sheet_names$i)_$layer "
   ...
   puts "OVERWRITING results to $Data(Zone_name)_sandwich_$Data(Res)m_$Data(AutoTEB_version).tif"
   file delete -force ./$Data(Zone_name)_sandwich_$Data(Res)m_$Data(AutoTEB_version).tif
   gdalfile open FILEOUT write ./$Data(Zone_name)_sandwich_$Data(Res)m_$Data(AutoTEB_version).tif GeoTiff
   gdalband write RSANDWICH FILEOUT { COMPRESS NONE PROFILE GeoTIFF }
   gdalfile close FILEOUT
...
proc AutoTEB::ChampsBuffers { } {
   ...
   #puts "Writing results to $Data(Zone_name)_champs_buf100ma+25mp_$Data(Res)m_$Data(AutoTEB_version).tif"
   #file delete -force ./$Data(Zone_name)_champs_buf100ma+25mp_$Data(Res)m_$Data(AutoTEB_version).tif
   #gdalfile open FILEOUT write ./$Data(Zone_name)_champs_buf100ma+25mp_$Data(Res)m_$Data(AutoTEB_version).tif GeoTiff
   ...
   puts "Writing results to $Data(Zone_name)_champs-only+building-vicinity_$Data(Res)m_$Data(AutoTEB_version).tif"
   file delete -force ./$Data(Zone_name)_champs-only+building-vicinity_$Data(Res)m_$Data(AutoTEB_version).tif
   gdalfile open FILEOUT write ./$Data(Zone_name)_champs-only+building-vicinity_$Data(Res)m_$Data(AutoTEB_version).tif GeoTiff
   gdalband write RBUFFERCUT FILEOUT { COMPRESS NONE PROFILE GeoTIFF }
   gdalfile close FILEOUT
   gdalband free RBUFFERCUT
}

proc AutoTEB::PopDens2Builtup { } { 
   variable Data
   set Data(Pop_file) /home/afsr/alx/eveil/canyon-urbain/global_data/statcan/traitements/da2001ca_socio_eco.shp
   ...
   puts "Writing results to $Data(Zone_name)_popdens_$Data(Res)m_$Data(AutoTEB_version).tif"
   file delete -force ./$Data(Zone_name)_popdens_$Data(Res)m_$Data(AutoTEB_version).tif
   gdalfile open FILEOUT write ./$Data(Zone_name)_popdens_$Data(Res)m_$Data(AutoTEB_version).tif GeoTiff
   gdalband write RPOPDENS FILEOUT { COMPRESS NONE PROFILE GeoTIFF }
   gdalfile close FILEOUT
   ogrlayer free VPOPDENS
   ogrfile close SHAPE

   #puts "Write results to $Data(Zone_name)_popdens_$Data(AutoTEB_version).shp"
   #file delete -force ./$Data(Zone_name)_popdens_$Data(AutoTEB_version).shp
   #ogrfile open VPOPDENSFILE write ./$Data(Zone_name)_popdens_$Data(AutoTEB_version).shp "ESRI Shapefile"
   ...
   puts "Writing results to $Data(Zone_name)_popdens-builtup_$Data(Res)m_$Data(AutoTEB_version).tif"
   file delete -force ./$Data(Zone_name)_popdens-builtup_$Data(Res)m_$Data(AutoTEB_version).tif
   gdalfile open FILEOUT write ./$Data(Zone_name)_popdens-builtup_$Data(Res)m_$Data(AutoTEB_version).tif GeoTiff
   gdalband write RPOPDENSCUT FILEOUT { COMPRESS NONE PROFILE GeoTIFF }
   gdalfile close FILEOUT
   gdalband free RPOPDENSCUT
}

proc AutoTEB::HeightGain { } {
   variable Data
   puts "\nEvaluating height gain"   
   gdalband read RCHAMPS [gdalfile open FCHAMPS read $Data(Zone_name)_champs-only+building-vicinity_$Data(Res)m_$Data(AutoTEB_version).tif]
   ...
   puts "Writing results to $Data(Zone_name)_hauteur-champs_$Data(Res)m_$Data(AutoTEB_version).tif"
   file delete -force ./$Data(Zone_name)_hauteur-champs_$Data(Res)m_$Data(AutoTEB_version).tif
   gdalfile open FILEOUT write ./$Data(Zone_name)_hauteur-champs_$Data(Res)m_$Data(AutoTEB_version).tif GeoTiff
...
#   puts "Writing results to $Data(Zone_name)_hauteur-builtup+building_$Data(Res)m_$Data(AutoTEB_version).tif"
#   file delete -force ./$Data(Zone_name)_hauteur-builtup+building_$Data(Res)m_$Data(AutoTEB_version).tif
#   gdalfile open FILEOUT write ./$Data(Zone_name)_hauteur-builtup+building_$Data(Res)m_$Data(AutoTEB_version).tif GeoTiff
#   gdalband write RHAUTEURCUT FILEOUT { COMPRESS NONE PROFILE GeoTIFF }
#   gdalfile close FILEOUT
   puts "Writing results to $Data(Zone_name)_hauteur-classes_$Data(Res)m_$Data(AutoTEB_version).tif"
   file delete -force ./$Data(Zone_name)_hauteur-classes_$Data(Res)m_$Data(AutoTEB_version).tif
   gdalfile open FILEOUT write ./$Data(Zone_name)_hauteur-classes_$Data(Res)m_$Data(AutoTEB_version).tif GeoTiff
...
proc AutoTEB::Values2TEB { } {
   ...
   gdalband read RSANDWICH [gdalfile open FSANDWICH read $Data(Zone_name)_sandwich_$Data(Res)m_$Data(AutoTEB_version).tif]
   gdalband read RPOPDENSCUT [gdalfile open FPOPDENSCUT read $Data(Zone_name)_popdens-builtup_$Data(Res)m_$Data(AutoTEB_version).tif]
   gdalband read RCHAMPS [gdalfile open FCHAMPS read $Data(Zone_name)_champs-only+building-vicinity_$Data(Res)m_$Data(AutoTEB_version).tif]
   gdalband read RHAUTEURCLASS [gdalfile open FHAUTEURCLASS read $Data(Zone_name)_hauteur-classes_$Data(Res)m_$Data(AutoTEB_version).tif]
...
#   gdalband define RTEB -georef [gdalband define RSANDWICH -georef]   
   ...
   puts "Writing results to $Data(Zone_name)_TEB_$Data(Res)m_$Data(AutoTEB_version).tif"
   file delete -force ./$Data(Zone_name)_TEB_$Data(Res)m_$Data(AutoTEB_version).tif
   gdalfile open FILEOUT write ./$Data(Zone_name)_TEB_$Data(Res)m_$Data(AutoTEB_version).tif GeoTiff
...
proc AutoTEB::VegeMask { } {
   variable Data
   puts "\nGenerating vegetation mask"
   gdalband read RTEB [gdalfile open FTEB read $Data(Zone_name)_TEB_$Data(Res)m_$Data(AutoTEB_version).tif]
   ...
   set fileRTEBfilter $Data(Zone_name)_vegemask-$Data(VegeFilterType)$Data(VegeFilterSize)_$Data(Res)m_$Data(AutoTEB_version).tif
   if { ![file exists $fileRTEBfilter] } {
      if { $Data(VegeFilterSize) > 20 } {
         puts "Generating the mask may require hours to process"
      }
      vector create FILTER [AutoTEB::FilterGen $Data(VegeFilterType) $Data(VegeFilterSize)]
...
#      vexpr VEGEMASK fcentile(RTEBWMASK,3,0.5) ;# fcentile is fmedian, fmax, fmin à la fois
   ...
   puts "Writing results to $Data(Zone_name)_TEB-wVegeMask_$Data(Res)m_$Data(AutoTEB_version).tif"
   file delete -force ./$Data(Zone_name)_TEB-wVegeMask_$Data(Res)m_$Data(AutoTEB_version).tif
   gdalfile open FILEOUT write ./$Data(Zone_name)_TEB-wVegeMask_$Data(Res)m_$Data(AutoTEB_version).tif GeoTiff
...
proc AutoTEB::CreatefstdBand { Name Band } {
...
proc AutoTEB::TEB2fstd { } {
   variable Data
   puts "\nConverting TEB raster to .fstd"
   gdalband read BAND [gdalfile open FILE read ./$Data(Zone_name)_TEB_$Data(Res)m_$Data(AutoTEB_version).tif]
   AutoTEB::CreatefstdBand GRID BAND
   fstdfield define GRID -NOMVAR UG
   file delete -force ./$Data(Zone_name)_TEB_$Data(Res)m_$Data(AutoTEB_version).fstd
   fstdfile open 1 write $Data(Zone_name)_TEB_$Data(Res)m_$Data(AutoTEB_version).fstd
...
proc AutoTEB::Shp2Height { } {
   variable Data
   puts "\nConverting $Data(Zone_name) building shapefile to raster"
   if { $Data(Zone_name) == "ott" } {
      set shp_file /home/afsr/alx/eveil/canyon-urbain/global_data/cities/ottawa/ott-buildings.shp
   }
   gdalband read RSANDWICH [gdalfile open FSANDWICH read $Data(Zone_name)_sandwich_$Data(Res)m_$Data(AutoTEB_version).tif]
   ...
   puts "Writing results to /home/afsr/alx/eveil/canyon-urbain/global_data/cities/ottawa/ott_shp-height.tif"
   file delete -force /home/afsr/alx/eveil/canyon-urbain/global_data/cities/ottawa/ott_shp-height.tif
   gdalfile open FILEOUT write /home/afsr/alx/eveil/canyon-urbain/global_data/cities/ottawa/ott_shp-height.tif GeoTiff
   gdalband write RHAUTEURSHP FILEOUT { COMPRESS NONE PROFILE GeoTIFF }
   gdalfile close FILEOUT
   gdalband free RHAUTEURSHP
}

proc AutoTEB::FilterGen { Type Size } {
...
AutoTEB::AreaParameters
AutoTEB::FindZoneUTM
AutoTEB::DefineZone
AutoTEB::FindNTSSheets
AutoTEB::FindPaths

#AutoTEB::SandwichBNDT         ;# ATTENTION - TRAITEMENT TRES LONG SI MODE = INCLUDED
if { ($AutoTEB::Data(Res) <= 5) && ($AutoTEB::Data(Mode) == "FAST") } {
#   AutoTEB::ScaleBuffers
}
if { $AutoTEB::Data(Zone_name) == "ott"} {
## La rasterization des hauteurs n'a pas vraiment d'affaire dans AutoTEB... C'est one-shot.
##   AutoTEB::Shp2Height
}
#AutoTEB::ChampsBuffers
#AutoTEB::PopDens2Builtup
#AutoTEB::HeightGain
#AutoTEB::BuildingHeight ;# This proc must be used in conjuction with the previous one otherwise $Data(HeightGain) won't be defined

#AutoTEB::Values2TEB
AutoTEB::VegeMask
##AutoTEB::TEB2fstd  ;# CAN'T WORK WITH FILES OVER 128 MEGS (e.g. MTL, VAN, TOR)
